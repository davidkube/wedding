...
export function Hero() {
  ...
  return (
    <div id="top">
      <section ref={bandRef} className="relative z-10 overflow-hidden bg-wine px-[clamp(16px,4vw,64px)] pb-[clamp(60px,9vw,120px)] pt-[clamp(104px,15vw,180px)] text-oat">
        <MarkPattern className="z-0" />
        <div className="relative z-[1] flex flex-col md:flex-row md:items-stretch md:gap-[clamp(16px,2.4vw,32px)]">
          <div className="md:w-[78%]">
            <motion.div ref={blockRef} className="mx-auto max-w-[1280px]" style={reduce ? undefined : { y: blockY, opacity: blockOpacity }}>
              <SplitLines
            ...
            <div className="relative mx-auto mt-[clamp(20px,3vw,44px)] max-w-[1120px]">
              <motion.div
          ...
          <div className="mt-[clamp(20px,3vw,44px)] md:mt-0 md:w-[22%]">
            <div className="relative aspect-[3/4] w-full overflow-hidden border border-ink/15 md:h-full md:aspect-auto">
              <video
                className="h-full w-full object-cover"
                src={hero.sideVideo.src}
                aria-label={hero.sideVideo.alt}
                autoPlay
                muted
                loop
                playsInline
